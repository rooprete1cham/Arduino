...
{
  if( (left_sonar>= 0)&&(left_sonar <= WALL_DISTANCE)&&(front_sonar>= 0)&&(front_sonar <= WALL_DISTANCE)&&(right_sonar>= 0)&&(right_sonar <= WALL_DISTANCE)  )
  {  //3면이 다막혔을때
    maze_status = 4;
    Serial.println("maze_status = 4");
  }
  else if( (left_sonar>= 0)&&(left_sonar <= WALL_DISTANCE)&&(right_sonar>= 0)&&(right_sonar <= WALL_DISTANCE)   )
  {
    maze_status = 1;
    Serial.println("maze_status =1");
  }
  else if( (left_sonar>= 0)&&(left_sonar <= WALL_DISTANCE)&&(front_sonar>= 0)&&(front_sonar <= WALL_DISTANCE)   )
  {
    maze_status = 2;
    Serial.println("maze_status = 2");
  }
  else if( (right_sonar>= 0)&&(right_sonar <= WALL_DISTANCE)&&(front_sonar>= 0)&&(front_sonar <= WALL_DISTANCE) )
  {
    maze_status = 3;
...
void loop()
{
  ...
  else if(maze_status == 3)
  {
    ...
    motor_B_control(HIGH,0);    
    delay(1000);
    motor_A_control(HIGH,140);   //오른쪽 전진
    motor_B_control(LOW,110);    //왼쪽 후진
    delay(400);
    ...
    else if(maze_status == 2)
  {
    ...
    motor_B_control(HIGH,0);    
    delay(1000);
    motor_A_control(LOW,120);   //오른쪽 전진
    motor_B_control(HIGH,110);    //왼쪽 후진
    delay(640);
    motor_A_control(HIGH,0);  
    motor_B_control(HIGH,0);    
    delay(1000);
