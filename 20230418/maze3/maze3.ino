...
void check_maze_status(void)
{
 /* if( (left_sonar>= 0)&&(left_sonar <= WALL_GAP_DISTANCE_HALP)&&(front_sonar>= 0)&&(front_sonar <= WALL_GAP_DISTANCE_HALP)&&(right_sonar>= 0)&&(right_sonar <= WALL_GAP_DISTANCE_HALP)  )
  ...
  if( (left_sonar>= 0)&&(left_sonar <= WALL_GAP_DISTANCE_HALP)&&(right_sonar>= 0)&&(right_sonar <= WALL_GAP_DISTANCE_HALP) )
  {
    maze_status = 1;
...
//먼저 left_pwm = 0;  ringht_pwm = 100; 으로 해서 왼쪽 오른쪽 방향 찾기

void wall_collinion_avoid(int base_speed)
  {
  float error = 0.0;
  float Kp = 0.65;       // 나중에 조정해야 할 값
  int pwm_control = 0;
  int right_pwm = 0;
  int left_pwm = 0;
  ...
  motor_B_control(HIGH,left_pwm);    //왼쪽 후진
}
void loop() 
{
